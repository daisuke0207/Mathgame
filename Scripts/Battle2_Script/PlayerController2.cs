...
public class PlayerController2 : MonoBehaviour
{
    ...
    //二つ目のリスト
    List<GameObject> removeBallList2 = new List<GameObject>();
    List<int> removeBallSum2 = new List<int>();
    ...
    private IEnumerator OnClickEnd()
    {
        ...
        if (ballSum == GameUI2.clearNumber || ballSum == GameUI2.cleatNumber2)
        {
            for (int i = 0; i < remove_cnt; i++)
            {
                ...
                GameUI2.BigBallGauge();
                if (GameUI2.bigballGauge.GetComponent<Image>().fillAmount >= 1)
                {
                    BallDirecotr.DropBigball(bigball, lastBall);
                    GameUI2.bigballGauge.GetComponent<Image>().fillAmount = 0;
                }
                yield return new WaitForSeconds(0.05f);
                Destroy(Deseffect);
            }
            if (ballSum == GameUI2.clearNumber) { GameUI2.Number_Random(); }
            else if (ballSum == GameUI2.cleatNumber2) { GameUI2.Number2_Random(); }
        }
        else
        {
            for (int i = 0; i < remove_cnt; i++)
